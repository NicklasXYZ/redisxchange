...
# An example script showing how to use the FIFO message queue wrapper class    #
# implemented in the 'redisxchange' library.                                   #
#                                                                              #
# The messaging flow can be summarized as follows:                             #
# 1. Messages are put into a named FIFO message queue.                         #
# 2. A client then takes a message from the queue and produces a response.     #
# 3. The client makes the response available to the sender of the message in   #
#    in the Redis' Key-Value store using a unique identifier that the sender   #
...
class Producer:
    def __init__(self, name: str) -> None:
        ...
        self.exchange = RedisQueueMessageExchange()
...
if __name__ == "__main__":       
    ...
    # - Consume messages from 'queue-one'
    # - And make results available on the Key-Value store
    consumer = Consumer("queue-one")

    # Create and start producer
    # - Put messages in 'queue-one'
    # - Retrieve messages from the Key-Value store
